...
//                数字+字母+特殊符号
                    if(this.newPassWord.length>=6 && this.newPassWord.length<=16){
                        ...
                        if(reg.test(this.newPassWord)){
                            ...
                            this.$refs.new_tit.innerHTML="密码必须为6-16位数字字母特殊符号";
                        }
                    }else if(this.newPassWord.length==0){
                        this.$refs.new_tit.innerHTML="请输入新密码";
                        this.newPassTogg=true;
                    }else{
                        this.$refs.new_tit.innerHTML="密码长度小于6位，请重新输入";
                        this.newPassTogg=true;
                    }
                },
            // 验证确认新密码是否为空
            checkAgain(){
                ...
                }else if(this.againNewPassWord.length>=6 && this.againNewPassWord===this.newPassWord){
                    this.$refs.again_tit.innerHTML="请再次输入密码";
                    this.surePassTogg=false;
                }else if(this.againNewPassWord.length>=6 && !(this.againNewPassWord===this.newPassWord)){
                    this.$refs.again_tit.innerHTML="两次密码输入不一致";
                    this.surePassTogg=true;
                }else if(this.againNewPassWord.length<6){
                    this.$refs.again_tit.innerHTML="密码长度小于6位，请重新输入";
                    this.surePassTogg=true;
                }
            },
            initInputType(){
...
//                              sessionStorage.setItem('userInfo', JSON.stringify({userToken:response.data.data.token}));
								this.$message({
                                    message: '密码已修改成功',
                                    type: 'success'
                                });
        ...
        watch:{
            ...
            againNewPassWord:function(){
                if(this.againNewPassWord.length>=6 && this.againNewPassWord===this.newPassWord){
                    this.$refs.again_tit.innerHTML="请再次输入新密码";
                    this.surePassTogg=false;
                    return true;
                }else if(this.againNewPassWord.length>=6 && !(this.againNewPassWord===this.newPassWord)){
